...
main_output_directory<-args[1]
core_number<-args[2]
save_intermediate_objects<-args[3]
...
output_folder<-paste0(main_output_directory, "final_output/")

old_new_names<-read.table(file=paste0(main_output_directory, "combined_targets/names_dictionary.tab"), header = T, sep="\t")
...
ref_genomes_directory<- paste0(main_output_directory, "output_per_genome")
genome_paths<-list.files(path=ref_genomes_directory, full.names=T)
cat("file list length is:", length(genome_paths), sep="\n")
paths<-""
pathnames<-""
for (j in 1:length(genome_paths)) {
    pathnames[j]<-basename(genome_paths[j])
    paths[j]<-paste0(genome_paths[j], "/blastdbcmd_output/")
    cat(paths[j])
}
